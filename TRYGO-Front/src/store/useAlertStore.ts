...
export const Alert: AlertType = (type, text, title, timeout, id) =>
  useAlertStore.getState().Alert(type, text, title, timeout, id);
export const closeAlert = (timeoutId: number | NodeJS.Timeout) =>
  useAlertStore.getState().closePopup(timeoutId);
