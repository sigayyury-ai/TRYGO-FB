...
async function testSocketIOMessage(token, projectId, hypothesisId) {
    ...
    return new Promise((resolve, reject) => {
        ...
        const timeout = setTimeout(() => {
            if (!messageReceived) {
                socket.disconnect();
                reject(new Error('Timeout: No response received within 120 seconds'));
            }
        }, 120000);
        ...
        socket.on('connect', () => {
            ...
            setTimeout(() => {
                ...
                const testMessage = {
                    message: 'What is a Lean Canvas and how can it help me validate my business idea? Please provide a detailed explanation with examples.',
                ...
                console.log('   Hypothesis ID:', hypothesisId);
                console.log('   Question: What is a Lean Canvas and how can it help me validate my business idea?');
                console.log('   Waiting for AI response... (this may take up to 2 minutes)');
        ...
        socket.on('answerCreated', (data) => {
            ...
            console.log('   Answer preview (first 500 chars):', data?.message?.substring(0, 500) || 'No message');
            ...
            if (!data?.message || data.message.length === 0) {
                ...
                console.warn('⚠️  Warning: Response seems too short. Expected detailed answer.');
