...
import { generateSecurePassword } from '../utils/password/generateSecurePassword';
...
class UserService {
    ...
    /**
     * Creates a new user account with an auto-generated secure password
     * Used for embedded onboarding widget
     * 
     * @param email - User's email address
     * @param role - User role (defaults to UserRole.User)
     * @returns Object containing the created user and the generated password (plain text, not stored)
     */
    async registerWithAutoGeneratedPassword(
        email: string,
        role: UserRole = UserRole.User
    ): Promise<{ user: IUser; generatedPassword: string }> {
        try {
            const existingUser = await this.getUserByEmailDefault(email);
            if (existingUser) {
                throw new Error('User with this email already exists');
            }
            ...
            // Hash the password
            const passwordHash = await createPasswordHash(generatedPassword);

            // Set free trial expiration (7 days from now)
            const sevenDaysFromNow = new Date(
                Date.now() + 7 * 24 * 60 * 60 * 1000
            );
            ...
            return {
                user: newUser,
                generatedPassword, // Return plain password for email sending (not stored)
            };
        } catch (error) {
            console.error('Error creating user with auto-generated password:', error);
            throw error;
        }
    }
}
